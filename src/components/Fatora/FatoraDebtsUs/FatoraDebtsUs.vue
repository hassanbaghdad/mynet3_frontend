    ...
    <v-container>
        <v-card :loading="$store.state.loading">
            <v-card-title>
                ...
                <span class="mr-2 f18">ديون بيع لنا</span>
            </v-card-title>
            <v-divider/>
            <v-card-text>
...
<script>
    ...
    export default {
        ...
        data() {
            return {
                debts: this.$store.state.fatora_debts_to_us.debts,
                customLabels,
                customStyles,
                pageOfItems: [],
                search:{
        ...
        methods: {
            ...
            search_debt()
            {
                var filtered = this.$store.state.fatora_debts_to_us.debts;
        ...
        computed: {
            get_debts: function () {
                return this.$store.state.fatora_debts_to_us.debts;
            },
        ...
        watch: {
            get_debts: function (new_debts) {
                ...
                var debts = this.$store.state.fatora_debts_to_us.debts;
                this.sum_total = 0;
                this.sum_wasel = 0;
                this.sum_carry = 0;
                debts.map(x=>{
